...
def _collate_morph_tag_data_samples(morph_df: pd.DataFrame, tag2index: dict):
    sent_groups = list(morph_df.groupby([morph_df.sent_id]))
    num_sentences = len(sent_groups)
    token_groups = list(morph_df.groupby([morph_df.sent_id, morph_df.token_id]))
...
def load_morph_vocab(data_path: Path, partition: dict):
    ...
    feats.add('_')
    tags.add('_')
    feats = ['<pad>'] + sorted(list(feats))
    tags = ['<pad>'] + sorted(list(tags))
...
def save_morph_tag_data_samples(data_path: Path, morph_partition: dict, tag2index: dict):
...
def load_morph_tag_data_samples(data_path: Path, morph_partition: list):
    tag_samples_partition = {}
    for part in morph_partition:
        tag_samples_file = data_path / f'{part}_morph_tag_data_samples.csv'
...
def load_morph_tag_data(data_path: Path, partition: list):
    morph_tag_data_samples = load_morph_tag_data_samples(data_path, partition)
    arr_data = {}
    for part in partition:
