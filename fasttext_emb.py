...
def load_embedding_weight_matrix(model_file_path: Path, vec_file_path: Path, words: list) -> nn.Embedding:
    if vec_file_path.exists():
        ...
        if len(new_words) > 0:
            new_word_vectors = _get_word_vectors(model_file_path, new_words)
            word_vectors.extend(new_word_vectors)
    else:
        word_vectors = _get_word_vectors(model_file_path, words)
        if words[0].lower() == '<pad>' or words[0].lower() == '[pad]':
...
def _get_word_vectors(model_path, words: list) -> list:
    global ft_model
    if not ft_model:
